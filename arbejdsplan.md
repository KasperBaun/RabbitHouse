# Arbejdsplan — Kaninhus

Trin i byggerækkefølge, grupperet pr. byggesystem. Hus-zonen (X = 0..1500).
Løbegården er en separat struktur og står til sidst.

## 0. Forberedelse & opmåling ✅

- [x] Vælg + ryd placering, ret terræn af
- [x] Mål op, sæt galger og snor (2000 × 3000 mm)
- [x] Marker hjørner ved (0,0), (2000,0), (2000,3000), (0,3000)
- [x] Afsæt grøftebredde til udgravning

## 1. Fundament ✅

- [x] Udgrav rendegrøft ~30 cm bred × ~80 cm dyb
- [x] Stabilgrus 100 mm i bund, komprimer
- [x] Læg fundablok-ring, 4 skifter halvstensforbandt
- [x] Vandret armering bund + top, lodret i hjørner
- [x] Udstøb hulrum med beton, lad hærde
- [x] Sokkelpuds på blokke. Kontroller om hele fundamentet er vandret, og ret evt. af med mørtel inden pudsning.
- [x] Støb klaplag i fundamentets indre.

## 2. Gulv (gulv-overkant flugter sokkel-top, z=120) → [kort & opmåling](docs/arbejdsplan/gulv-stroer-lem.md)

- [x] Kant-bærereglar V3/V5 45×95 i Y (2700) på ringen (overkant z95)
- [x] Mellem-bærereglar i Y + for/bag-reglar + veksler om lemmene
- [x] ~25 mm gulvbrædder i X på reglarne (overkant z120, flugter sokkel-top)
- [ ] Lemme (fals + låg + greb) + trapper ned i kælder → [sådan bygger du dem](docs/arbejdsplan/lem-og-trappe.md)

## 3. Konstruktions-skelet (vægge)

- [ ] Monter bundrem, murpap og ankerbolte → [detaljer](docs/arbejdsplan/bundrem-murpap-ankerbolte.md)
- [ ] Rejs V3 (venstre)
- [ ] Rejs V1 (front, m. dør- + 2 vindue-åbninger)
- [ ] Rejs V2 (bag)
- [ ] Rejs V5 (partition, m. hus-dør + pet-dør)
- [ ] Læg topremme på alle 4 vægge
- [ ] Vinkelbeslag i alle hjørner

## 4. Tagkonstruktion

- [ ] Rejs spær m. hanebånd + fugleudskæring
- [ ] Vindskeder ved gavl-spær
- [ ] Sternbrædder langs perimeter
- [ ] Sofitt under tagskæg

## 5. Tagdækning (skifer)

- [ ] Undertag
- [ ] Lægter
- [ ] Læg skifer-plader
- [ ] Rygning + inddækninger
- [ ] Tagrende + nedløb

## 6. Udvendig beklædning

- [ ] Vindpapir på alle vægge, stift c/c 200
- [ ] Cut åbninger i vindpapir
- [ ] Afstandslister c/c 600
- [ ] Beklædning (vertical_120)
- [ ] Hjørnetrim ved 4 hjørner

## 7. Åbninger

- [ ] Sæt karme i rough openings
- [ ] Yard-udhusdør (Å1, mesh-leaf)
- [ ] Indvendig dør (Å2)
- [ ] Side-vindue plexi (Å3)
- [ ] Kattelem i pet-åbning
- [ ] Hængsler + greb + lås

## 8. Isolering

- [ ] Mineraluld i væg-cavity
- [ ] Dampspærre
- [ ] Isoler gulv mellem strøer (valgfrit)

## 9. Indvendig beklædning

- [ ] Plader indvendigt på vægge

## 10. Inventar

- [ ] Rede-kasse
- [ ] Høhæk
- [ ] Foder-/vandskåle

## 11. Overfladebehandling

- [ ] Træbeskyttelse/maling udvendigt
- [ ] Behandling indvendigt
- [ ] Slut-rengøring + indflytning

---

## Løbegård (separat struktur)

- [ ] Fundament
- [ ] Skelet (V4 + V1/V2-segmenter)
- [ ] Voliere-net (front + bag + højre)
- [ ] Yard-dør
- [ ] Tag-låg (mesh)
