# Render each house roof_cover option and save comparison images.
$ErrorActionPreference = "Stop"
$env:Path += ";C:\Program Files\OpenSCAD"
$out = "_renders/roof_compare"
New-Item -ItemType Directory -Force -Path $out | Out-Null

$covers = "skifer","tagpap","eternit"

foreach ($c in $covers) {
    $output = & openscad -o "$out/$c.png" --imgsize=1200,800 --colorscheme=Tomorrow `
        --camera=3000,1500,1300,55,0,25,16000 `
        -D "house_roof_cover=`"$c`"" src/main.scad 2>&1
    $errs = $output | Select-String 'ERROR|Assertion' | Where-Object { $_ -notmatch 'viewport' }
    if ($errs) {
        Write-Host "WARNING: house_roof_cover='$c' produced errors:" -ForegroundColor Yellow
