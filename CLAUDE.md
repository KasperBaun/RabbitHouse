...
- **v3** — ONE continuous mono-pitch roof over the entire 6 m × 2,5 m footprint, sloping front-to-back (eh_front=2600, eh_back=2200, drop=400 over 2500 mm = 16 % slope). The house occupies X=0..1500 (1.5 m); the yard occupies X=1500..6000 (4.5 m). Foundation is a continuous fundablok strip (50×20×15 cm blocks, 3 courses in halvstensforbandt = ~60 cm tall) under ALL walls — perimeter ring + cross-wall under the partition at X=hl — sitting on stabilgrus in a frostfri trench (~80 cm dig). Drawn by `fundablok_ring(ll, ww, 3, [hl])` in `lib/primitives/fundablok.scad`; top of foundation at Z=0 (grade), depth 600 mm. Use `show_ground=false` in `main.scad` to hide the grass and inspect the buried foundation. The yard sits on grass at grade with concrete ground plugs still drawn at the right-end corners (legacy — the fundablok ring already carries those post lines and the plugs could be removed in a follow-up). House side and partition walls are mono-pitch cladded with a sloped top beam below the roof. Yard back wall has a low solid-clad strip (driving-rain skirt) topped by a mesh ventilation band. No play-things in the yard — minimalist grass + bowls + 2 rabbits.
