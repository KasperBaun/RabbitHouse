...
(V1 front, V2 bag, V5 højre), lagt i **halvstensforbandt med vekslende
hjørne-ejerskab** — samme mønster som [hus-fundamentet](../hus/fundament.md).

I **kombineret build** med hus'et danner hus'ets V4-væg (ved X = 2000)
...
```
   ...
   Y=3000 ┌─────────────────────────────────────────┐  ← V2 bag (yard-segment)
          ...
          │       Y = 1000..3000                    │    (V5 højre)
          │                                         │
          │       (V4 = husets højre væg → fælles)  │
...
| Venstre væg (V4) | I COMBINED: ejet af hus. I STANDALONE: render egen strip |

## Halvstensforbandt — hjørner skifter ejer per skifte

Hjørnerne ved V5 (front-right og back-right) interlocker perpendikulært
...
den **korte væg** (V5 — 2000 mm, fordi yard-dybden Y = 1000..3000) der ejer
hjørnerne på lige skifter.

| Skifte | Lange vægge (V1, V2 — 4000 mm) | Kort væg (V5 — 2000 mm) |
|--------|--------------------------------|--------------------------|
...
I STANDALONE-mode tilføjes V4 venstre væg (samme dimensioner som V5 — kort,
2000 mm): den ejer hjørner sammen med V5 på lige skifter, og sidder mellem
...
| Lodret | **4–6 stk** korte stænger (~600 mm) | I V5-hjørner + et par midt på V1/V2 — bindes til top og bund banderne |
...
| 2 | Fundablok 50 × 20 × 15 cm | 4 skifter halvstensforbandt på 3-sidet ring (V4 er hus'ets) | 86 | stk (80 + 6 buffer) |
...
| 6 | Ankerskruer M10 × 120 | Bundrem-til-ring: 4 på V1, 4 på V2, 2 på V5 | 10 | stk |
...
|---|---|---|
| Fundablok extra | +16 stk | Venstre strip ved X = 2000 (4 skifter × 4 blokke; V4 = 2000 mm) |
| Kamstål Ø10 mm × 3 m extra | +2 stk | V4-bane: 1 bund + 1 top à 2 m (lodret = genbrug egne stænger) |
| Cement extra | +2 sæk | Ekstra hulrum (~60 L beton) |
| Stabilgrus extra | ~100 kg | Ekstra V4-grøft 2 m × 30 cm × 10 cm |
| Ankerskruer M10 extra | +2 stk | V4 venstre væg (Y = 1500, 2500) |
...
6. Læg **2. skifte** (lige — V5 ejer hjørner): V5 fuld Y = 1000..3000. V1 og V2
   mellem X = 2000..5850. Standalone: V4 også fuld Y = 1000..3000.
7. **Sæt 4–6 lodrette Ø10 mm-stykker (~600 mm)** ned gennem hulrum i V5-
...
12. Bor og sæt M10 ankerskruer c/c 1000 mm i topskiftet:
    ...
    - V5 (højre): y = 1500, 2500 (2 stk; yard-dybde 2000 mm rummer kun 2)
    - Standalone V4 (venstre): y = 1500, 2500

## Rendering / verificering

```powershell
# Render combined build (hus + yard) for at se hvordan yard's fundament
# slutter op mod hus'ets V4 ved X = 2000.
...
- V5 højre fundament løber fra Y = 1000 til Y = 3000 i lige skifter,
  fra Y = 1150 til Y = 2850 i ulige skifter (interlocker med V1/V2)
- I COMBINED ingen venstre væg (V4 er hus'ets); i STANDALONE er der egen V4-strip
  med samme alternation som V5
